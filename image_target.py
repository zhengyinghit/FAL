...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        features_test = netB(netF(inputs_test))  # netF: B x 2048, netB: B x 256
        outputs_test = netC(features_test)

        idx, counts = torch.unique(mem_label, return_counts=True)
        M_c = torch.mean(counts.float())
        weight = torch.ones(args.class_num).cuda()
        for i in range(len(idx)):
            weight[idx[i]] = M_c / counts[i]

        criterion = FAL(weight=weight, num_classes=args.class_num)

        pred = mem_label[tar_idx]
        classifier_loss = criterion(outputs_test, pred)
...
def obtain_label(loader, netF, netB, netC, args):
    ...
    if args.dset == 'sentiment':
        ...
        return torch.from_numpy(predict.astype('int')).cuda()


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='FAL')
    ...
    parser.add_argument('--dset', type=str, default='sentiment',
                        choices=['sentiment', 'emotion', 'office-home'])
    ...
    parser.add_argument('--output', type=str, default='ckps/fal')
